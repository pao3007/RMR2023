...
float polohaX = 0, polohaY = 0, angle = 0;
bool moving = false;
  ...
  //  if(datacounter%5)
    {
        ...
        if((rightWheel != 0) || (leftWheel != 0)){
            moving = true;
        }else if((rightWheel == 0) && (leftWheel == 0)) moving = false;
...
int MainWindow::processThisLidar(LaserMeasurement laserData)
{
    ...
    double pi1 = 3.14159265359;
    float rads, lidarAngle, calAngle;
    if(angle < 0){
        rads = angle + 6.283185;
    }else rads = angle;
    ...
    int Ygi;
    double scanDist;
    emit uiValuesChanged(polohaX, polohaY,angle);

    if(!moving){
        for(int k=0;k<copyOfLaserData.numberOfScans;k++){
            scanDist = copyOfLaserData.Data[k].scanDistance / 1000;
            lidarAngle = copyOfLaserData.Data[k].scanAngle * (pi1/180.0);
            //if(lidarAngle > 6.283185/2) lidarAngle -= 6.283185;
            calAngle = -rads + lidarAngle;
            if(calAngle > 6.283185) calAngle -= 6.283185;

            if(scanDist > 0){
                Xgi = gSize/2 + (int)((polohaX + scanDist*cos(calAngle))/0.05);
                Ygi = gSize/2 + (int)(-(polohaY - scanDist*sin(calAngle))/0.05);
                occ_grid[Xgi][Ygi] = 1;
            }
        ...
        ofstream outfile("C:/Users/pao/Desktop/RMR/RMR2023/uloha3/mapa.txt");

            // Write occupancy grid data to file
            for (int i = 0; i < gSize; i++) {
                for (int j = 0; j < gSize; j++) {
                    outfile << occ_grid[j][i] << " ";
                }
                outfile << endl;
            }
